      ...
      <div className="space-y-4 px-4 py-4">
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
          <div className="flex items-start gap-4">
            <div className="w-16 h-16 rounded-2xl bg-primary-light text-primary flex items-center justify-center text-lg font-bold flex-shrink-0">
              {worker.avatar_url ? (
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className={cn(
                      ...
                      isSaved ? 'bg-red-50 border-red-200 text-red-500' : 'border-gray-200 text-gray-400'
                    )}
                  >
          ...
          <div className="grid grid-cols-3 gap-3 mt-5">
            <div className="text-center p-3 bg-gray-50 rounded-xl">
              ...
              <p className="text-[10px] text-gray-500 mt-0.5">Jobs Done</p>
            </div>
            <div className="text-center p-3 bg-gray-50 rounded-xl">
              <p className="text-lg font-bold text-primary">{formatPKR(worker.hourly_rate)}</p>
              <p className="text-[10px] text-gray-500 mt-0.5">Per Hour</p>
            </div>
            <div className="text-center p-3 bg-gray-50 rounded-xl">
              <p className="text-lg font-bold text-primary">{formatPKR(worker.daily_rate)}</p>
              <p className="text-[10px] text-gray-500 mt-0.5">Per Day</p>
            </div>
          </div>

          {worker.is_available && (
            <div className="flex items-center gap-2 mt-4 p-2.5 bg-green-50 rounded-xl">
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
          ...
          <div className="flex flex-wrap gap-2">
            {worker.skills.map((skill) => (
              <span key={skill} className="px-3 py-1.5 bg-primary-light text-primary rounded-full text-xs font-medium">
                {skill}
              </span>
            ))}
          </div>
                ...
                <div key={review.id} className="border-b border-gray-50 pb-3 last:border-0 last:pb-0">
                  ...
                  <div className="flex items-center gap-0.5 mb-1.5">
                    {[1, 2, 3, 4, 5].map((s) => (
                      <Star
                        key={s}
                        className={cn('w-3 h-3', s <= review.rating ? 'text-amber-400 fill-amber-400' : 'text-gray-200')}
                      />
                    ))}
                  </div>
        ...
        <div className="flex gap-3 pb-4">
          <button
            onClick={() => router.push(`/post-job?category=${encodeURIComponent(worker.category)}`)}
            className="flex-1 bg-primary hover:bg-primary-hover text-white py-3.5 rounded-2xl font-semibold text-sm flex items-center justify-center gap-2 transition-colors"
          >
            <Briefcase className="w-4 h-4" /> Invite to Job
          </button>
          <button className="w-14 h-14 flex items-center justify-center rounded-2xl border-2 border-primary text-primary">
